...
const THUMB_MAP_FILE = path.join(ROOT, 'src', 'data', 'sportThumbs.ts');
const META_FILE = path.join(IMG_DIR, 'attributions.json');

// Widths fetched per sport. FULL backs the sport-page hero, which runs the
// full width of the screen; THUMB backs the home grid, where cards are about
// 170pt wide. A require()d image decodes at its stored size regardless of how
// small it is drawn, so serving the hero asset to the grid meant decoding
// ~1000px bitmaps for 40 cards and the scroll stuttered.
...
const SEARCH_TERMS = {
  ...
  kickboxing: 'kickboxing training gym bag',
  ...
  aikido: 'aikido demonstration dojo',
  fencing: 'fencing competition',
  kendo: 'kendo kenshi armor practice',
  archery: 'archer shooting bow arrow target',
  running: 'marathon runners race',
  swimming: 'swimming pool competition swimmer',
  ...
  'olympic-weightlifting': 'weightlifting competition barbell athlete',
  crossfit: 'crossfit athlete workout gym',
  gymnastics: 'artistic gymnastics rings',
  yoga: 'yoga practice pose',
  soccer: 'soccer player ball match',
  ...
  badminton: 'badminton player racket court',
  climbing: 'rock climbing bouldering',
  hiking: 'hiker mountain trail backpack',
  surfing: 'surfing wave surfer',
  skiing: 'skiing snow slope skier',
  snowboarding: 'snowboarder snow slope',
  skateboarding: 'skateboarder skatepark',
};
...
async function searchCommons(term) {
  const url =
    ...
    // 1000px keeps the sport-page hero crisp on 3x-density phones; the grid
    // gets a THUMB_WIDTH copy derived from this URL below.
    `&prop=imageinfo&iiprop=url|extmetadata|size&iiurlwidth=${FULL_WIDTH}`;
...
for (const sport of SPORTS) {
  ...
  try {
    ...
    const name = `${sport.id}.${extOf(hit.thumbUrl)}`;
    const file = path.join(IMG_DIR, name);
    fs.writeFileSync(file, Buffer.from(await res.arrayBuffer()));

    // Commons encodes the width into its thumbnail paths (".../1000px-X.jpg"),
    // so the smaller copy is the same URL with the width swapped — no local
    // image library needed. Falls back to the full asset if that 404s.
    const thumbUrl = hit.thumbUrl.replace(`/${FULL_WIDTH}px-`, `/${THUMB_WIDTH}px-`);
    const thumbFile = path.join(THUMB_DIR, name);
    if (thumbUrl !== hit.thumbUrl) {
      const tRes = await fetchWithBackoff(thumbUrl);
      if (tRes.ok) {
        fs.writeFileSync(thumbFile, Buffer.from(await tRes.arrayBuffer()));
      } else {
        console.warn(`${sport.id}: no ${THUMB_WIDTH}px thumb, reusing full size`);
...
writeMap(MAP_FILE, 'sportImages', 'assets/sports');
writeMap(THUMB_MAP_FILE, 'sportThumbs', 'assets/sports/thumbs', [
  `${THUMB_WIDTH}px versions of the same photos, for the home grid. The full-size`,
...
console.log('wrote ATTRIBUTIONS.md, sportImages.ts and sportThumbs.ts');
