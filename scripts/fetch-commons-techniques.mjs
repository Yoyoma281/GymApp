...
const RAW_BASE = 'https://raw.githubusercontent.com/Yoyoma281/GymApp/main/assets/techniques';
...
for (const [sport, drills] of Object.entries(TECHNIQUES)) {
  ...
  for (const [drillId, title] of Object.entries(drills)) {
    ...
    drill.clipUrl = `${RAW_BASE}/${sport}-${drillId}.mp4`;
    drill.clipPoster = `${RAW_BASE}/${sport}-${drillId}.jpg`;
    drill.clipCredit = `${info.author} / Wikimedia Commons, ${info.license}`;
    // Marks a clip that shows this exact technique rather than the sport in
    // general, so the UI can say so and a future re-pick won't overwrite it.
    drill.clipIsTechnique = true;
